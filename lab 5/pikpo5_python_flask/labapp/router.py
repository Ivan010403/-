...
def put_data_into_db(request):
    sqlite_connection = sqlite3.connect('C://Users//vanyk//OneDrive//Рабочий стол//Учёба//ПИКПО//SWD&C Lab4//pikpo4_python//test.db')
    cursor = sqlite_connection.cursor()

    final = request.split()

    sql_update_query = """Update processed_data set rain = ? where id = ?"""
    data = (final[1], final[0])
    cursor.execute(sql_update_query, data)

    sql_update_query = """Update processed_data set temperature = ? where id = ?"""
    data = (final[2], final[0])
    cursor.execute(sql_update_query, data)

    sql_update_query = """Update processed_data set wind_speed = ? where id = ?"""
    data = (final[3], final[0])
    cursor.execute(sql_update_query, data)
